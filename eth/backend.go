...
type Dogeum struct {
	...
	lock sync.RWMutex // Protects the variadic fields (e.g. gas price and ethervase)
...
func New(stack *node.Node, config *ethconfig.Config) (*Dogeum, error) {
	...
	eth := &Dogeum{
		...
		ethervase:         config.Miner.Etherbase,
...
func (s *Dogeum) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
	ethervase := s.ethervase
	s.lock.RUnlock()

	if ethervase != (common.Address{}) {
		return ethervase, nil
	}
	if wallets := s.AccountManager().Wallets(); len(wallets) > 0 {
		if accounts := wallets[0].Accounts(); len(accounts) > 0 {
			ethervase := accounts[0].Address

			s.lock.Lock()
			s.ethervase = ethervase
			s.lock.Unlock()

			log.Info("Etherbase automatically configured", "address", ethervase)
			return ethervase, nil
		}
	}
	return common.Address{}, fmt.Errorf("ethervase must be explicitly specified")
}

// isLocalBlock checks whether the specified block is mined
// by local miner accounts.
//
// We regard two types of accounts as local miner account: ethervase
...
func (s *Dogeum) isLocalBlock(header *types.Header) bool {
	...
	// Check whether the given address is ethervase.
	s.lock.RLock()
	ethervase := s.ethervase
	s.lock.RUnlock()
	if author == ethervase {
...
func (s *Dogeum) SetEtherbase(ethervase common.Address) {
	s.lock.Lock()
	s.ethervase = ethervase
	s.lock.Unlock()

	s.miner.SetEtherbase(ethervase)
}
...
func (s *Dogeum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		eb, err := s.Etherbase()
		if err != nil {
			log.Error("Cannot start mining without ethervase", "err", err)
			return fmt.Errorf("ethervase missing: %v", err)
		}
