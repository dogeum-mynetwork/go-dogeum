...
// setEtherbase retrieves the ethervase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setEtherbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *ethconfig.Config) {
	// Extract the current ethervase
	var ethervase string
	if ctx.IsSet(MinerEtherbaseFlag.Name) {
		ethervase = ctx.String(MinerEtherbaseFlag.Name)
	}
	// Convert the ethervase into an address and configure it
	if ethervase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, ethervase)
			if err != nil {
				Fatalf("Invalid miner ethervase: %v", err)
			}
			cfg.Miner.Etherbase = account.Address
		} else {
			Fatalf("No ethervase configured")
		}
	}
}
