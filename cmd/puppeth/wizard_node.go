...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an ethervase to mine against
			fmt.Println()
			if infos.ethervase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ethervase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.ethervase)
				infos.ethervase = w.readDefaultAddress(common.HexToAddress(infos.ethervase)).Hex()
			}
